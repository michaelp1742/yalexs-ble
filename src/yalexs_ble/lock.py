...
# Budget of 12 s for the op-response, measured from the command write like the
# acknowledgement budget. Measured the same way in the logs, op-responses land
# within 5.75 s at the 95th percentile; the longer tail there is a saturated
# link rather than a slow lock, so the budget is sized off the body of that
# distribution and not off its maximum.
OPERATION_RESPONSE_TIMEOUT = 12.0
# Unlatch is the longest motion (retract the deadbolt, pull the spring latch
# in, hold it, release it) and there is no field capture of one, so it gets a
# longer budget than a lock or an unlock. The motion is expected to end inside
# the plain 12 s; the extension is head-room for a motion nobody has measured,
...
def _ack_matcher(opcode: int, operation_byte: int) -> Callable[[bytes], bool]:
    ...
    byte[1], and the command's operation byte in byte[4] (plain operations
    carry 0x00 there, securemode 0x04, unlatch 0x0A). Any other frame is not
    this command's acknowledgement; it stays on the state-callback path and
    the wait continues.

    The 0x00 and 0x04 echoes are field captured. The 0x0A echo is inferred
    from the same rule, since no unlatch has been captured: a lock that
    answered an unlatch with a zero byte[4] would fail this match and the
    operation would time out at the acknowledgement stage.
    """
...
class Lock:
    def __init__(
        ...
        # for, None between operations. An op-response this does not match was
        # produced by an operation started at the lock, in the app, or by
        # auto-lock, or by an operation of ours whose wait already ended: the
        # wait clears this field on its way out, so a late op-response arrives
        # with nothing awaited. Nothing on our side reports its result.
            ...
            if (
                ...
                if result != OperationError.COMM_SUCCESS:
                    ...
                    if solicited and result in MECHANICAL_OPERATION_ERRORS:
                        ...
                        # lock, in the app, or by auto-lock, or one of ours
                        # whose wait already ended and cleared the awaited
                        # opcode before this late op-response landed. Nothing
                        # else reports these cases, so this is their only
                        # record.
                        _LOGGER.warning(
                            "%s: Operation failed with result 0x%02X (%s)",
                            self.name,
                    ...
                    # Every failure class needs manual intervention at the
                    # lock, so all of them display as JAMMED.
    ...
    async def _execute_operation_command(
        ...
        failure (a MECH_* code is a motor stall, a jam).
        """
        assert self.session is not None  # nosec
        opcode = command[0x01]
        ...
        if result != OperationError.COMM_SUCCESS:
            ...
            raise OperationFailedError(
                ...
                f"({error.name if error else 'unknown'})",
                result,
            )
    ...
    async def force_unlatch(
        ...
        UnlatchError (OperationIncompleteError and OperationFailedError,
        already non-retryable, propagate as themselves). Failures before the
        write attempt (connect, session setup, encryption) stay retryable.

        That same rule makes the acknowledgement a don't care here, so this is
        the one operation that waits on its op-response alone. For lock and
        unlock a missing acknowledgement ends the attempt early and the retry
        re-sends; no re-send is allowed here, and the latch answers about
        10 seconds after the command, well past the acknowledgement budget, so
        waiting on the acknowledgement could only end the operation while the
        latch was still out and leave its op-response arriving with nothing to
        receive it.
        """
        ...
        try:
            ...
            # Already non-retryable: the result never arrived, or it arrived
            # and named a failure. Ordered ahead of the broad clause below so
            # both types reach the caller unwrapped.
            raise
    ...
    async def lock(self) -> None:
        """Lock; raises OperationFailedError on a reported failure.
    ...
    async def unlock(self) -> None:
        """Unlock; raises OperationFailedError on a reported failure.
    ...
    async def unlatch(self) -> None:
        ...
        # always fires (lock/unlock short-circuit when already in the target
        # state; there is no "unlatched" steady state to test).
