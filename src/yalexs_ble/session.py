...
class UnlatchError(YaleXSBLEError):
    ...
    lock, and a repeated unlatch fires the latch again -- the door opens again
    -- so no failure from that point on may re-send it. Deliberately NOT a
...
class OperationProgress:
    ...
    a command keys on write_attempted, the earlier of the two marks.
    command_written records confirmed delivery; it currently has no consumer
    and is retained as a hook for a caller that needs to tell a delivered
    command from an attempted one.
...
class Session:
    ...
    def _notify(self, char: int, data: bytearray) -> None:
        ...
        try:
            ...
            if self._operation_progress is None and self._notify_future is not None:
                ...
                if self._notify_future.done():
                    # The wait ended before this frame arrived: its timeout
                    # cancelled the future, and the waiting task has not resumed
                    # yet to disarm the slot. Resolving a future that is already
                    # done raises InvalidStateError into the notify dispatcher,
                    # so the slot is left to the waiter's own cleanup.
                    return
        ...
        if self._notify_future.done():
            ...
            # answer the command. The record above is taken first, so a wait
            # that ended in this turn still reports the result it was waiting
            # for.
            return
    ...
    async def _locked_write_operation(
        ...
        try:
            ...
            # The ATT write response came back, so the command is confirmed
            # delivered. Recorded for callers that distinguish attempted from
            # delivered; nothing in this chain reads it yet.
