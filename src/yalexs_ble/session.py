...
class Session:
    ...
    def _reject_frame(self, ex: ResponseError, frame: bytes | bytearray) -> None:
        ...
        The frame is withheld from the state callback. A plain solicited wait
        still receives the error, so _locked_write re-sends its command as it
        always has; an unsolicited frame has no waiter and is dropped.
        """
        # The drop line carries the frame hex at INFO: these frames should not
        # occur at all, so their evidence sits above the debug noise floor, and
        # the logged stream stays complete for a full-frame survey.
    ...
    def _notify(self, char: int, data: bytearray) -> None:
        ...
        if len(data) != RESPONSE_FRAME_LEN:
            ...
            self._reject_frame(
                ResponseError(f"{len(data)} bytes is not a response frame"), data
            )
            return
        ...
        if self._notify_future.done():
            ...
            # for. The waiter reports its own timeout, and that report is all a
            # reader of the log would otherwise see, so the frame that would
            # have answered it is named here.
            _LOGGER.debug(
