...
class Session:
    ...
    def _reject_frame(self, ex: ResponseError, frame: bytes | bytearray) -> None:
        ...
        wait receives the error, so _locked_write re-sends its command. A wait
        that has already ended is left to its own cleanup, and an unsolicited
        frame has no waiter at all; either way the frame is dropped.
        """
