...
class PushLock:
    ...
    def __init__(
        ...
        # admitted jam event pushes it out again (NEVER_TIME = no hold). The
        # deadline survives a reconnect, so a hold spans the connections it
        # outlives; its wakeup is a host-side timer and survives them too.
        # Outside this initialization, the deadline and the wakeup are
        # written only through _arm_jam_hold and _release_jam_hold, so the
        # two cannot diverge.
