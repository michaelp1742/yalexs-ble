...
class PushLock:
    ...
    def _operation_write_success(self) -> None:
        ...
        A jam still on record goes with the hold, and for the same reason: the
        record can only survive to here on a command a caller issued after the
        jam, which is the manual intervention a jam calls for, and its outcome
        is the newer truth.
        """
    ...
    async def _execute_lock_operation(
        ...
        try:
            ...
            self._close_operation_window()
            # Heal sooner than the keep-alive cadence: a transitional may be
            # on display, the link is up, and no result is coming, so the
            # sooner a read replaces it the better. The delay is the settle
    ...
    def _merge_cycle_state(
        self, state: LockState, lock_status_fetched: bool
    ) -> LockState:
        """Merge this cycle's readings with what the notify path published.

        Notify callbacks land while the cycle is awaiting its own reads, so the
        cached state can be newer than the values this cycle started from.
        """
        ...
        # polled nothing carries its cached value forward untouched, and the
        # value admitted is the one the poll actually returned, before the
        # UNKNOWN carry-forward below can substitute the cached status. Only a
        # status read from the lock this cycle is jam evidence: a cached
        # JAMMED substituted for a polled UNKNOWN is the display's own value
        # coming back around, and admitting it would extend its own hold for
        # as long as the lock keeps answering UNKNOWN.
        ...
        # during awaited operations in this update cycle. This substitution is
        # display only: it runs after the admission above, so a carried-forward
        # status never re-arms the jam hold deadline or its wakeup.
    ...
    async def _update(self) -> LockState:
        ...
        state = self._merge_cycle_state(state, lock_status_fetched)
        ...
        if state.lock in (LockStatus.UNKNOWN_01, LockStatus.UNKNOWN_06):
            ...
            # forced reconnect is deliberately suppressed: the Home Assistant
            # integration has displayed 01 and 06 as jammed since its initial
            # mapping, and a reconnect is not a valid response while jammed is
            # on display.
