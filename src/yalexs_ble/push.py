...
class PushLock:
    ...
    def _operation_write_success(self) -> None:
        ...
        A command that reaches the lock supersedes any jam still on record:
        the record can only survive to here on a command a caller issued after
        the jam, which is the manual intervention a jam calls for, and its
        outcome is the newer truth.
        """
        self._seen_jam = False
    ...
    def _merge_cycle_state(self, state: LockState) -> LockState:
    ...
    async def _update(self) -> LockState:
        ...
        state = self._merge_cycle_state(state)
