...
# How long a fresh status needing someone at the lock stays pinned on display,
# as a deadline that every admitted event of that class pushes out again (now +
# JAMMED_HOLD_TIME, the _poll_battery deadline pattern). Post-jam GETSTATUS
# fabricates a plain position (14/14 field jams; JAMMED was clobbered in under
# a second), and the end of a jam cannot be determined remotely, so the hold's
# primary purpose is making the user aware of it via the UI. The setup
# conditions have no capture of their own and are held on the same rule, since
# neither of them ends without a person at the lock either. The hold masks only
# the reported lock status and schedules no poll while it runs, so it has no
# connection side effect; a wakeup at the hold's end schedules the follow-up
# poll that reads the lock's current state. 30 s is a nominal visibility
# window.
...
class _DeadlineWakeup:
    ...
    the deadline is read again, and a deadline that moved out after the wakeup
...
class PushLock:
    ...
    # Declared here rather than in __init__ because the initializers that set
    # them, _init_operation_state and _init_jam_state, are defined below the
    ...
    def _operation_write_success(self) -> None:
        ...
        Clearing _seen_intervention_status is the backstop for its invariant,
        that every operation exit applies a status it recorded, so no record
        reaches a new command's write-success. A record that did reach here
        would answer the same way the hold does: the command a caller issued
        after it is the intervention that status calls for, and its outcome is
        the newer truth.
        """
    ...
    def _close_operation_window(self) -> None:
        ...
        _seen_intervention_status is cleared here rather than at each exit that
        applies it, so the record cannot outlive the window that took it
        whatever the exit. _settle_operation reads it into the outcome before
        calling this, and the retry path reaches this only when it is already
        clear, so nothing that owes the display a status loses one to the line
        below.
        """
        self._operation_window_open = False
        self._pending_op_state = None
    ...
    def _settle_operation(self) -> None:
        ...
        A display hold still live from an earlier status of that class outranks
        the outcome too, by the admission rule rather than here: it holds that
        status on display, so the unknown position an unlatch that failed at
        its write asks for is masked. That is the same ruling as the recorded
        status above, for the same reason, and it is bounded: the hold's wakeup
        removes LockStatus from _seen_this_session and asks the lock again when
        the hold lapses.
        ...
        if not self._running:
            ...
            # neither action below is wanted now: both arm a timer holding the
            # lock past the stop, the booking its own and a jam outcome the
            # display hold's, with nothing left to run either.
            return
    ...
    def _init_jam_state(self) -> None:
        """Initialise everything that carries a held status between events.

        _jammed_hold_deadline is the moment a held status stops being pinned;
        every admitted event of that class pushes it out again (NEVER_TIME = no
        hold). The deadline and its wakeup survive a reconnect: a mechanism
        needing attention outlives the link that reported it, and the wakeup is
        a host-side timer. They do not survive a stop, which releases the hold
        (see _cancel). _seen_intervention_status carries the status the
        operation window filtered out until the operation applies it, the
        status itself rather than a flag so the display carries the one the
        lock reported, and it is cleared when the window closes, so it is
        discharged well before any reconnect could reach it. Outside this
        initialisation the deadline and the wakeup are written only through
        _arm_jam_hold and _release_jam_hold, so the two cannot diverge.
        """
    ...
    def _jam_hold_ended(self) -> None:
        """The display hold has ended: poll the lock's current state.

        None of these conditions announces its end remotely, so once the hold
        has lapsed the held status is only a guess and one follow-up poll is
    ...
    def _admit_lock_status(
        ...
        if self._operation_window_open:
            ...
            if incoming in MANUAL_INTERVENTION_STATUSES:
                # A dropped status of this class is not assumed to arrive
                # again. In the jams captured from another central the failure
                # report and the position settle behind it landed within a
                # couple of seconds of each other, close enough for one window
                # to swallow both, and no later frame in those captures carried
                # the jam. The setup conditions have no capture at all, so they
                # are recorded on the same rule rather than on evidence of
                # their own. The record is what lets the operation apply the
                # status at its own exit.
        ...
        now = time.monotonic()
        if incoming in MANUAL_INTERVENTION_STATUSES:
            # Admitted evidence that the lock needs attention, whatever the
            # bearer (our own op-response applied after the window closes, a
            # settled 0x07 push, a poll answer, a foreign failure op-response):
            # every event sets a new hold end. It is a real reading of the
            # position, so _seen_this_session keeps LockStatus; the wakeup
            # armed here removes it and schedules the follow-up poll once the
            # hold actually ends (see _jam_hold_ended). A status the window
            # filter dropped is discharged here, whichever path carried it
            # back: it has reached the display.
            self._seen_intervention_status = None
            self._arm_jam_hold(now)
            return incoming
        if current in MANUAL_INTERVENTION_STATUSES and now < self._jammed_hold_deadline:
            # Display hold: after one of these the polled register fabricates a
            # plain locked/unlocked position and no remote signal marks the
            # condition's end, so the status stays pinned until the deadline or
            # until a new operation's write-success releases it. Masking
            # schedules nothing: the hold's own wakeup carries the follow-up
            # poll, so the hold has no connection side effect.
            _LOGGER.debug(
    ...
    async def _execute_lock_operation(
        ...
        try:
            ...
            if self._seen_intervention_status is not None:
                ...
                # just reported needs attention; that work is a person's at the
                # lock. OperationIncompleteError carries that to the
    ...
    def _cancel(self) -> None:
        self._running = False
        self._cancel_future_update()
        # The hold exists to make a user aware of a jam through the UI, and a
        # stopped watcher has no display to hold it on. Releasing it takes the
        # wakeup with it, which would otherwise fire after the stop and book a
        # cycle on a lock nothing is watching. Leaving the deadline behind
        # instead would mask the lock's status on a restarted watcher with no
        # wakeup left to end the hold or to ask the lock again.
        self._release_jam_hold()
