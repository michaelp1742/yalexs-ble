...
@pytest.mark.asyncio
async def test_stop_cancels_the_hold_end_wakeup():
    """Stopping the lock cancels the hold's ending wakeup, so it cannot fire
    after the stop and book a cycle on a lock nothing is watching."""
...
@pytest.mark.asyncio
async def test_deferred_update_backs_off_while_an_operation_holds_the_lock():
    """The counterpart of the test above: the deadline has lapsed the same way,
    but an operation holds the lock, so the cycle backs off instead of queueing
    behind it."""
