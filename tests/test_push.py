...
async def test_a_new_commands_write_success_clears_the_jam_record() -> None:
    ...
    The record outlives the operation that took it on the exits that apply no
    status at all (a bare cancel, exhausted authentication attempts), so the
    next command a caller issues is the manual intervention a jam calls for
    and its own result is what the display carries.
    """
    push_lock = _operational_push_lock()
...
# ---------------------------------------------------------------------------
# Hold JAMMED on display at both state-application sites
# ---------------------------------------------------------------------------
...
@pytest.mark.asyncio
async def test_a_polled_unknown_after_the_hold_keeps_the_display_and_arms_nothing():
    """Past the hold's end a poll answering UNKNOWN still shows the cached
    JAMMED (the display-only carry-forward), and no new hold is armed."""
...
@pytest.mark.asyncio
async def test_merge_rescues_unknown_lock_and_door_from_the_cache():
    """A cycle that fetched nothing keeps the cached lock and door for display
    when its own working state holds UNKNOWN."""
    push_lock = _operational_push_lock("aa:bb:cc:dd:ee:50")
    push_lock._lock_state = _known_state(LockStatus.LOCKED, door=DoorStatus.CLOSED)

    working = _known_state(LockStatus.UNKNOWN, door=DoorStatus.UNKNOWN)
    merged = push_lock._merge_cycle_state(working, lock_status_fetched=False)

    assert merged.lock is LockStatus.LOCKED
    assert merged.door is DoorStatus.CLOSED
