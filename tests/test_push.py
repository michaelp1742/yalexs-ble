...
@pytest.mark.asyncio
async def test_update_reconnects_when_the_lock_reports_an_unknown_position() -> None:
    """A pushed 0x01 or 0x06 still forces a reconnect, now read from the state."""
    push_lock = PushLock(
    ...
    mock_lock = MagicMock()
    mock_lock.lock_status = publishing_read(push_lock, LockStatus.UNKNOWN_01)
    mock_lock.door_status = publishing_read(push_lock, DoorStatus.CLOSED)
    mock_lock.auto_lock_status = AsyncMock(return_value=None)
    mock_lock.battery = publishing_read(
        push_lock, BatteryState(voltage=6.0, percentage=80)
    ...
    forced_disconnect.assert_awaited_once()
    assert "unknown state" in forced_disconnect.await_args_list[0].args[0]
...
async def test_failure_op_response_applies_jammed_after_window():
    ...
    # runs _complete_operation with the same now, so the stale-state floor, the
    # disconnect timer and the keep-alive all move despite the jam.
...
async def test_cancelled_mid_operation_displays_a_jam_it_received() -> None:
    ...
    # The heal poll this exit books reads the fabricated position back, and the
    # hold masks it.
    with (
        patch.object(push_lock, "_ensure_connected", AsyncMock(return_value=mock_lock)),
        patch.object(
...
@pytest.mark.parametrize(
    ...
    [
        None,
        OperationFailedError("reported failure", 0x1F),
...
async def test_every_operation_exit_arms_the_heal_poll(
    ...
    the cadence an always-connected lock polls at anyway. The failure cases
    raise, so each runs its own handler and the read-back must be armed there.
    """
...
async def test_cancellation_heals_sooner_than_the_keep_alive() -> None:
    ...
    return the pre-operation position. The booking is taken after the
    command-issue and acknowledgement anchors are stamped, so it already
    clears the stale-state floor those anchors set.
    """
...
async def test_the_hold_does_not_keep_the_link_up_on_a_demand_lock():
    ...
    This pins the absence of the measured defect shape: with the read-back
...
async def test_forced_reconnect_suppressed_during_jammed_hold(bad):
    """A polled 01/06 during a JAMMED hold is masked to JAMMED as its frame
    lands, so the forced reconnect is deliberately suppressed."""
    ...
    async def lock_status() -> LockStatus:
        push_lock._update_any_state([bad])
        return bad
...
async def test_a_cycle_due_mid_operation_keeps_the_operations_outcome() -> None:
    ...
    A cycle booked while an operation runs reschedules to the deadline, 6.1 s
    past the acknowledgement. An unlatch answers about 10 s after the command,
    so that moment falls due with the latch still out. Queueing the cycle there
    would poll the instant the op-response landed, reading back the position
    the lock still reports from before the operation and putting it on display
    in place of the outcome the operation just applied.
    """
    ...
    async def force_unlatch(write_success_callback: Callable[[], None]) -> None:
        ...
        stale = time.monotonic() - (LOCK_STALE_STATE_DEBOUNCE_DELAY + 1.0)
        push_lock._last_lock_operation_start_time = stale
        push_lock._last_lock_operation_acknowledged_time = stale
    ...
    # The retry finds the deadline pushed out by the op-response.
    with patch.object(
        push_lock, "_schedule_future_update_with_debounce"
    ) as mock_reschedule:
        push_lock._deferred_update()
    ...
    mock_reschedule.assert_called_once()
    assert mock_reschedule.call_args.args[0] == pytest.approx(
        POST_OP_RESPONSE_DEBOUNCE_DELAY, abs=0.1
    )
    push_lock._cancel_future_update()
    push_lock._cancel_disconnect_timer()
...
# ---------------------------------------------------------------------------
# Battery polling past the motor's voltage sag
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        None,
        OperationFailedError("force_lock reported failure", 0x1F),
        OperationIncompleteError("the op-response never arrived"),
        asyncio.CancelledError(),
        "connect-failure",
    ],
    ids=["success", "jam", "incomplete", "cancelled", "connect-failure"],
)
async def test_operation_defers_battery_poll_past_voltage_sag(failure):
    """Every exit of an operation attempt arms the post-op battery cooldown:
    success, a reported failure, a missing result, a cancel, and a connect
    failure that never reaches the write. The deferral sits in the finally
    clause, so no exit can skip it; _poll_battery then skips within the
    window and polls past it."""
    push_lock = _operational_push_lock("aa:bb:cc:dd:ee:38")
    ...
    if failure == "connect-failure":
        ensure_connected = AsyncMock(side_effect=RuntimeError("no adapter slot"))
    else:
        ensure_connected = AsyncMock(return_value=mock_lock)
    ...
    with (
        ...
        if failure is None:
            ...
            expected = RuntimeError if failure == "connect-failure" else type(failure)
